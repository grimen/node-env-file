        ...
        lines.forEach(function(line) {
            ...
            if (is_comment) {
                ...
                if(options.substitutions) {
                    // TODO: support full parameter substitution as documented here...
                    // http://www.tldp.org/LDP/abs/html/parameter-substitution.html

                    // for now, we expect the following format...
                    // MY_VAR=${env_var_to_use:-sustitution_if_not_defined}

                    var value_substitution = env_value.match(/\$\{([^:]+):-([^\}]+)\}/);

                    if(env_value && value_substitution && value_substitution.length >= 2) {
                        var value_parameter = value_substitution[1];
                        var existing_env_var = process.env[value_parameter];
                        var substitution = value_substitution[2];

                        // use the existing environment variable if available, else use the default from the .env file
                        env_value = (typeof existing_env_var !== 'undefined') ? existing_env_var : substitution;
                    }
                }
...
module.exports.substitutions = true
module.exports.data = {}
